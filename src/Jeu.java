...
class Jeu extends Program {
    ...
    String[][] chargerScores() {
        CSVFile scoresCsv = loadCSV("ressources/scores.csv");
    ...
    void enregistrerScores(Joueur joueur, String[][] scores){
        ...
        saveCSV(scores, "ressources/scores.csv");
    }
    ...
    Question[] chargerQuestions(String niveau, String theme) { // charge les questions du fichier csv dans un tableau
        String path =  "ressources/questions/" + niveau + "/" + theme + ".csv";
    ...
    String[] getNiveaux() {
        String[] niveaux = getAllFilesFromDirectory("ressources/questions");

        for(int idxNiv = 0; idxNiv < length(niveaux); idxNiv++) {
            if(length(getAllFilesFromDirectory("ressources/questions/" + niveaux[idxNiv])) == 0) {
                niveaux[idxNiv] = null;
            }
        }
